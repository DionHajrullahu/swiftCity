...
import {
  ...
  Moon, Sun, LogIn, MapPin, UtensilsCrossed, Compass, Sparkles, X,
} from "lucide-react";
...
function SearchResults({
  results, loading, query, onClose,
}: {
  results: Recommendation[]; loading: boolean; query: string; onClose: () => void;
}) {
    ...
    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50 max-h-96 overflow-y-auto">
      ...
      {!loading && results.length === 0 && (
        <div className="py-10 text-center text-gray-400 text-sm">
          No recommendations found. Try a different city or interest.
        </div>
      )}
      ...
      {!loading && results.map((r) => {
        ...
        return (
          <div key={r.id} className="px-5 py-4 hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0">
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-6 py-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/30 text-white text-sm font-medium hover:bg-white/20 transition-colors"
        >
          <LogIn size={14} />
          Reviewer Login
        </button>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto w-full">
          <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-tight mb-6">
            Discover cities through <span className="text-[#3bbfb3]">local eyes</span>
          </h1>
          ...
          <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto mb-6">
            <input
              ...
              className="w-full px-6 py-4 pr-16 rounded-2xl text-gray-800 bg-white shadow-2xl text-base outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-[#3bbfb3]"
              autoComplete="off"
            />
            <button
              type="submit"
              className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-[#3bbfb3] hover:bg-[#2da89d] rounded-xl flex items-center justify-center transition-colors"
            >
            ...
            />
            {/* Invisible backdrop to close results */}
            {showResults && (
          ...
          <div className="flex items-center justify-center gap-3 flex-wrap">
            ...
            {QUICK_CITIES.map((city) => (
              <button
                ...
                className="px-4 py-1.5 rounded-full border border-white/30 text-white text-sm hover:bg-white/10 transition-colors backdrop-blur-sm"
              >
                {city}
              </button>
            ))}
            ...
            <div className="lg:w-2/5 text-left">
              ...
              <ul className="grid grid-cols-2 gap-2">
                {FEATURED_CITIES.map((city) => (
                  <li key={city.name} className={`flex items-center gap-2 text-sm py-2 px-3 rounded-lg transition-colors ${darkMode ? "text-gray-300 bg-gray-800/50 hover:bg-gray-800" : "text-gray-700 bg-white/60 hover:bg-white"}`}>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURED_CITIES.map(({ name, country, image }) => (
              <article key={name} className="group relative rounded-2xl overflow-hidden cursor-pointer shadow-md hover:shadow-xl transition-shadow duration-300" style={{ aspectRatio: "4/3" }}>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  ...
                  <button className="flex items-center gap-1.5 text-white text-sm font-medium group-hover:gap-3 transition-all duration-200">
